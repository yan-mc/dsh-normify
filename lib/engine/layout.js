...
export function idFromLayoutPath(relPath) {
...
export async function listLayoutFiles(projectDir) {
    const out = [];
    await walkJson(join(projectDir, 'renders'), '', out);
    return out.sort();
}
/** 读取单个渲染数据文件；不存在返回 null。 */
export async function loadLayoutFile(projectDir, id) {
 ...
 */
export function l1ValidateLayout(data, id, children, siblingEdges, where) {
    const errors = [];
    const warnings = [];
    if (!isPlain(data)) {
    ...
    if (data.mode !== undefined) {
        if (typeof data.mode !== 'string' || !LAYOUT_MODES.includes(data.mode)) {
    ...
    if (data.max_columns !== undefined) {
        if (!Number.isInteger(data.max_columns) || data.max_columns < 1 || data.max_columns > 6) {
    ...
    if (data.max_api_rows !== undefined) {
        if (!Number.isInteger(data.max_api_rows) || data.max_api_rows < 0 || data.max_api_rows > 48) {
    ...
    if (data.edge_hints !== undefined) {
        if (!Array.isArray(data.edge_hints)) {
            ...
            for (const h of data.edge_hints) {
                ...
                const hint = { from, to };
                if (h.kind !== undefined) {
                    if (typeof h.kind !== 'string' || !DEP_KINDS.includes(h.kind)) {
                ...
                if (h.lane !== undefined) {
                    if (!Number.isInteger(h.lane) || h.lane < 0 || h.lane > 9) {
    ...
    const layout = {
        ...
        ...(mode !== undefined ? { mode } : {}),
        ...(data.max_columns !== undefined ? { max_columns: data.max_columns } : {}),
        ...(data.max_api_rows !== undefined ? { max_api_rows: data.max_api_rows } : {}),
...
export async function writeLayoutFile(projectDir, layout) {
...
export async function deleteLayoutFile(projectDir, id) {
    const path = layoutFilePath(projectDir, id);
    try {
...
export async function validateLayouts(projectDir, byId, childrenOf, siblingEdges) {
    const errors = [];
    const warnings = [];
    ...
    for (const rel of rels) {
        ...
        if (layout === null)
            continue;
        const r = l1ValidateLayout(layout, id, children, siblingEdges.get(id) ?? new Set(), 'renders/' + rel);
