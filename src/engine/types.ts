...
export const MODULE_STATES = ['active', 'planned', 'deprecated'] as const

export type ModuleState = (typeof MODULE_STATES)[number]

/** 架构规则的完整规则集（policy.yml）。 */
export const POLICY_RULE_TYPES = ['forbid-dependency', 'dependency-direction', 'acyclic', 'max-depth', 'cross-tree', 'naming'] as const
...
export const CHANGE_STATUSES = ['proposed', 'in_progress', 'verified', 'abandoned'] as const
...
export const LAYOUT_MODES = ['auto', 'layers', 'groups', 'grid'] as const
...
export interface LayoutData {
  ...
  max_columns?: number
  /** 叶子框内最多展示几行 API（0 = 全部展开，也是缺省）。项目越精细、每叶 API 越少，这一层越不需要截断。 */
  max_api_rows?: number
