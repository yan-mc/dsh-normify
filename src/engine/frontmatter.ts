...
import type { Api, Dep, Diagnostic, LocalizedText, Module, SourceRef } from './types.js'
...
function isL10n(v: unknown): v is LocalizedText {
  return v !== null && typeof v === 'object' && !Array.isArray(v)
    && typeof (v as Record<string, unknown>).zh === 'string'
...
export function checkSourceEntry(v: unknown, where: string, out: Diagnostic[]): boolean {
  ...
  for (const k of ['line', 'end_line'] as const) {
    const n = v[k]
...
export function checkApiEntry(v: unknown, where: string, out: Diagnostic[]): boolean {
  ...
  if (typeof proto !== 'string' || !PROTOCOLS.includes(proto as (typeof PROTOCOLS)[number])) {
...
export function checkDepEntry(v: unknown, where: string, out: Diagnostic[]): boolean {
  ...
  if (typeof v.kind !== 'string' || !DEP_KINDS.includes(v.kind as (typeof DEP_KINDS)[number])) {
  ...
  if (v.label !== undefined) checkL10n(v.label, 'label', 30, where, out)
  return true
}

/** L1：单文件级字段校验（规范 §5.2 结构/API/边类的格式部分）。 */
export function l1Validate(data: unknown, where: string): { module: Module | null; errors: Diagnostic[]; warnings: Diagnostic[] } {
  ...
  if (errors.length > 0) return { module: null, errors, warnings }
  const module: Module = {
    uid: uid as string,
    id: id as string,
  ...
  if (Array.isArray(data.apis)) {
    ...
    ;(data.apis as unknown[]).forEach((a, i) => checkApiEntry(a, where + '/apis/' + i, errors))
    if (errors.length > 0) return { module: null, errors, warnings }
  }
  if (Array.isArray(data.deps)) {
    module.deps = data.deps as Dep[]
    ;(data.deps as unknown[]).forEach((d, i) => checkDepEntry(d, where + '/deps/' + i, errors))
    if (errors.length > 0) return { module: null, errors, warnings }
  }
...
export function parseModuleText(text: string, where: string): { module: Module | null; body: string; errors: Diagnostic[]; warnings: Diagnostic[] } {
...
export function serializeModule(module: Module, body: string): string {
  ...
  if (module.repository !== undefined) out.push('repository: ' + plainScalar(module.repository))
  ...
  if (module.source.length === 0) {
    ...
    for (const s of module.source) {
      ...
      if (s.line !== undefined) out.push('    line: ' + s.line)
      if (s.end_line !== undefined) out.push('    end_line: ' + s.end_line)
    }
  }
  if (module.apis !== undefined) {
    if (module.apis.length === 0) {
      ...
      for (const a of module.apis) {
        ...
        if (a.method !== undefined) out.push('    method: ' + plainScalar(a.method))
  ...
  if (module.deps !== undefined && module.deps.length > 0) {
    ...
    for (const d of module.deps) {
      ...
      if (d.from_api !== undefined) out.push('    from_api: ' + q(d.from_api))
      if (d.to_api !== undefined) out.push('    to_api: ' + q(d.to_api))
      if (d.label !== undefined) out.push('    label: ' + l10nInline(d.label))
    }
  }
  out.push('---')
  const text = out.join('\n') + '\n'
