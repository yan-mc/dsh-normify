...
export async function renderProject(projectDir: string, opts: RenderOptions): Promise<RenderOutput> {
  ...
  try {
    ...
    return {
      ...
      sha256: null,
      summary: null,
      errors: [diag('error', 'render/no-build', '缺少 tree.json 编译产物', {}, {}, ['先运行 normify.build'])],
      warnings,
    }
  }
  ...
  try {
    ...
    return {
      ...
      sha256: null,
      summary: null,
      errors: [diag('error', 'render/bad-tree', 'tree.json 解析失败：' + String(error), {}, {}, ['重新运行 normify.build'])],
      warnings,
    }
  }
  const project = (tree.project ?? {}) as Record<string, unknown>
