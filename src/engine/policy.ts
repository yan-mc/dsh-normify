...
export function policyFilePath(projectDir: string): string {
    return join(projectDir, 'policy.yml');
}
/** id 模式匹配：`*` 恰一段、`**` 任意段（含零段），其余精确匹配。 */
export function matchIdPattern(pattern: string, id: string): boolean {
...
function anyMatch(patterns: string[] | undefined, id: string): boolean {
...
function effectiveState(m: Module): ModuleState {
    return m.state ?? 'active';
}
/** 默认规则模板（项目创建时写入；含两条启用的基础规则与全部类型的注释示例）。 */
export function defaultPolicyTemplate(): string {
...
function validPatternList(v: unknown): v is string[] {
    return Array.isArray(v) && v.length > 0 && (v as string[]).every(x => typeof x === 'string' && x.length > 0 && PATTERN_CHARS.test(x));
}
/** L1：policy.yml 结构与规则字段校验。 */
export function l1ValidatePolicy(data: unknown, where: string): { policy: PolicyData | null; errors: Diagnostic[]; warnings: Diagnostic[] } {
    ...
    for (const raw of data.rules as unknown[]) {
        ...
        seen.add(id);
        const type = raw.type;
        if (typeof type !== 'string' || !POLICY_RULE_TYPES.includes(type as PolicyRuleType)) {
        ...
        if (raw.kind !== undefined && (!Array.isArray(raw.kind) || (raw.kind as string[]).some(k => typeof k !== 'string' || !DEP_KINDS.includes(k as DepKind)))) {
            errors.push(diag('error', 'policy/rule-kind', 'kind 必须为 ' + DEP_KINDS.join(' | ') + ' 数组', { path: at }, {}, []));
        }
        for (const key of ['fromState', 'toState']) {
            if (raw[key] !== undefined && (typeof raw[key] !== 'string' || !MODULE_STATES.includes(raw[key] as ModuleState))) {
        ...
        if (ruleType === 'forbid-dependency') {
            ...
            if (raw.maxDepth !== undefined && (!Number.isInteger(raw.maxDepth) || (raw.maxDepth as number) < 1 || (raw.maxDepth as number) > 64))
                errors.push(diag('error', 'policy/rule-max-depth', 'maxDepth 必须为 1..64 整数', { path: at + '/maxDepth' }, { value: raw.maxDepth }, []));
        ...
        if (errors.length === errorsBefore)
            rules.push(raw as unknown as PolicyRule);
    }
    if (errors.length > 0)
        return { policy: null, errors, warnings };
    return {
...
export async function loadPolicyFile(projectDir: string): Promise<{ policy: PolicyData | null; exists: boolean; errors: Diagnostic[] }> {
    let text: string;
    try {
...
export async function writePolicyFile(projectDir: string, policy: PolicyData): Promise<string> {
...
/** 项目创建时安装默认规则（已存在则不覆盖）。 */
export async function installDefaultPolicy(projectDir: string): Promise<void> {
...
/** L2：执行全部启用的规则。 */
export function evaluatePolicy(policy: PolicyData, ctx: EvalContext): Diagnostic[] {
    const out: Diagnostic[] = [];
    for (const rule of policy.rules) {
        if (rule.enabled === false)
            continue;
        const severity = rule.severity ?? 'error';
        const push = (message: string, subject: Record<string, unknown>, evidence: Record<string, unknown>, fixes: string[]): void => {
        ...
        if (rule.type === 'forbid-dependency') {
            ...
            const layers: PolicyLayer[] = rule.layers ?? [];
