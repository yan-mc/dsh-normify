...
export async function validateProject(projectDir: string, opts: ValidateOptions): Promise<ValidateOutput> {
    ...
    const checkChain = (id: string, chain: string[]): void => {
        if (seen.has(id))
            return;
        const f = byId.get(id);
        if (f === undefined)
    ...
    // "API 直连"引导（warning，不阻断）：两端都声明了 API 的箭头却没锚定到具体 API ——
    // 渲染器只能把箭头落在框边，钉不到 API 行，白丢"精细"这一层价值（0.5.3 新增）。
    const unanchored: { from: string; to: string; kind: string }[] = [];
    for (const f of files) {
        for (const d of f.module.deps ?? []) {
            if (d.from_api !== undefined || d.to_api !== undefined)
                continue;
            const target = byId.get(d.to);
            if (target === undefined)
                continue; // 悬空边由前面报 error
            if ((f.module.apis ?? []).length === 0 || (target.module.apis ?? []).length === 0)
                continue; // 至少一端没有 API：本来就锚不了
            unanchored.push({ from: f.module.id, to: d.to, kind: d.kind });
        }
    }
    if (unanchored.length > 0) {
        ...
        warnings.push(diag('warning', 'dep/unanchored', '有 ' + unanchored.length + ' 条箭头可锚定到具体 API 但未锚定（如 ' + preview + (unanchored.length > 3 ? ' 等' : '') + '）：两端都声明了 API，补上 from_api/to_api 后箭头才会钉在 API 行上（图的"精细"价值在这里）', {}, { total: unanchored.length, examples, hint: '在源模块 deps 条目里补 from_api（本模块 API 键）与 to_api（目标模块 API 键）；两者都必须是模块自身 apis 里的键' }, ['给这些箭头补 from_api/to_api', '若这条边确实不该锚定，忽略该 warning 或用 policy 忽略']));
    }
    ...
    const siblingEdges = new Map<string, Set<string>>();
    ...
    let policy: PolicyData | null = policyResult.policy;
...
function expectedPathHint(file: string, id: string): string {
