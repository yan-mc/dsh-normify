...
export async function closeChange(projectDir: string, id: string, opts: CloseOptions): Promise<CloseResult> {
    ...
    if (repoRoot !== undefined) {
        const after: LandingRef[] = (await loadAllModulesForClose(projectDir)).files;
    ...
    return {
        ...
        file,
        refresh,
        build: (rebuilt.receipt?.stats ?? b.receipt?.stats ?? null) as Record<string, unknown> | null,
        render,
