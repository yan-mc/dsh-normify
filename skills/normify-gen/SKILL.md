...
| `id` | 路径式：`[a-z0-9][a-z0-9-]*` 段以 `.` 连接；**首段 = 树名**；含树名段 ≤ 12 段；全项目唯一。示例 `demo.order.checkout.payment` |
...
3. **逐层下钻（要精细）**：每层先 `normify_module_list` 看当前层，再逐个展开，直到叶子。
   ...
   - **目标深度**：小仓库 ≥3 层、中大型 ≥4–6 层、大仓库 6 层以上（硬上限 12 段）；不要停在"一层目录 = 一层模块"。
...
**规模与批次**：总量不设上限；用 `normify_module_batch` 分批提交（**单批 ≤40 个模块**，原子、失败回滚），可连续多批直至整棵树建完。每批保持"批内依赖自闭合、跨批依赖后补（`normify_module_patch`）"，不要因为轮次或 token 预算提前收尾。
...
- **id 命名**：从代码结构推导（目录/包/服务名转小写段）；同层名字唯一；总段数 ≤ 12；不要用数字开头段；最后一段避免用 `index`（会与容器 index.md 冲突）。
...
- 深度 ≤ 12：超限禁止下钻，提示合并子模块。
- 单批 ≤ 40 个模块（`normify_module_batch` 原子提交）；模块**总量不设上限**，粒度到单一功能单元，鼓励 100–1000+ 模块的精细结构。
